package utils
{
	...
	public class Utils
	{
		...
		static public function get_url(uri:String):String {
			return get_url_by_host(Config.HOST, uri);
		}
		
		static public function get_url_by_host(host:String,uri:String):String {
			var d:Date = new Date();
			var c:String = "?";
			if(uri.indexOf("?") > 0) c = "&";
			return host + uri + c + "postfix=" + d.fullYear + d.month + d.date + d.hours + d.minutes + d.seconds + d.milliseconds.toString();
		}
		...
		static private var gFilter:GlowFilter = new GlowFilter(0x5dd7fe,0,0,0);
		static public function neonFilter(color:uint = 0x5dd7fe):GlowFilter {
			if(color == 0x5dd7fe)
				return gFilter;
			return new GlowFilter(color,0,0,0);
		}
